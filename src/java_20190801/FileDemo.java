...
public class FileDemo {
	...
	public void changeFile(String path) {
		// path => c:\dev\test\jdk.exe => c:\dev\test\12325452346236.exe
		File f1 = new File(path);
	...
	public static void main(String[] args) {
		 ...
		 * "KB"); boolean isFile = f2.isFile(); if (isFile) { String path =
		 * f2.getPath();// f2의 전체 경로를 반환한다. if (path.endsWith("pdf")) {
		 * System.out.println("PDF 파일"); } else if (path.endsWith("exe")) {
		 * System.out.println("응용프로그램"); } else if (path.endsWith("txt")) {
		 * System.out.println("텍스트 문서"); } } else { System.out.println("파일폴더"); } File
		 * f3 = new File("c:\\dev\\test", "jdk.exe"); f2.renameTo(f3);// f2파일을 f3파일 이름으로
		 * 변경
		 * 
